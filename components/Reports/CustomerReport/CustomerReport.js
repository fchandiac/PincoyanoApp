...
export default function CustomerReport(props) {
  const { customerReport, startDate, endDate } = useAppContext();
      ...
      >
        <Grid container spacing={2}>
          <Grid item xs={12}>
            ...
            <Typography variant={"h5"}>{customerReport.title}</Typography>
            ...
            <Grid container spacing={1}>
              <Grid item xs={6}>
                <TotalSalesCard
                  ...
                  amount={customerReport.totalSales}
                />
              </Grid>
              <Grid item xs={6}>
                <TotalUnitsCard
                  title={"Unidades vendidas"}
                  quanty={customerReport.totalUnits}
                />
              </Grid>
            </Grid>
          </Grid>
          <Grid item xs={12}>
            <CategoriesBarChart
              data={customerReport.categoriesSales}
              title={"Ventas por categoría"}
            />
          </Grid>
          <Grid item xs={12} marginTop={2} className="break-after">
            <SalesLineChartCard
              data={customerReport.salesPerDay}
...
{/* 
          <Grid item xs={6}>
            <TopPieChart
              data={customerReport.topProductsAmount}
              title={"Productos más vendidos por monto"}
            />
          </Grid>
          <Grid item xs={6}>
            <TopPieChart
              data={customerReport.topProductsUnits}
